...
void VideoProcessingThread::run() {
    ...
    while (!m_stopRequested) {
        ...
        while (!m_hasPendingFrame && !m_stopRequested) { m_condition.wait(&m_mutex); }
            ...
            if (!image.isNull() && !targetSize.isEmpty()) {
                ...
                switch (static_cast<AdvancedCpp::QualityLevel>(quality)) {
                    case AdvancedCpp::QualityLevel::Low:
                        transformMode =
                            AdvancedCpp::QualitySettings<AdvancedCpp::QualityLevel::Low>::transformMode;
                        break;
                    case AdvancedCpp::QualityLevel::Medium:
                        transformMode =
                            AdvancedCpp::QualitySettings<AdvancedCpp::QualityLevel::Medium>::transformMode;
                        break;
                    case AdvancedCpp::QualityLevel::High:
                        transformMode =
                            AdvancedCpp::QualitySettings<AdvancedCpp::QualityLevel::High>::transformMode;
                        break;
                }
...
VideoWidgetQt::VideoWidgetQt(QWidget* parent)
    : QWidget(parent), m_mediaPlayer(std::make_unique<QMediaPlayer>(this)),
      m_videoSink(std::make_unique<QVideoSink>(this)), m_videoLabel(std::make_unique<QLabel>(this)),
      m_layout(std::make_unique<QVBoxLayout>(this)),
      m_processingThread(std::make_unique<VideoProcessingThread>(this)), m_duration(0.0), m_isLoaded(false),
      m_lastFrameTime(0), m_maxFrameRate(30), m_videoQuality(1)   // Medium quality by default
      ,
    ...
    AdvancedCpp::safeConnect(m_mediaPlayer.get(), &QMediaPlayer::mediaStatusChanged, this,
                             &VideoWidgetQt::onMediaStatusChanged);
    AdvancedCpp::safeConnect(m_mediaPlayer.get(), &QMediaPlayer::playbackStateChanged, this,
                             &VideoWidgetQt::onPlaybackStateChanged);
    AdvancedCpp::safeConnect(m_mediaPlayer.get(), &QMediaPlayer::positionChanged, this,
                             &VideoWidgetQt::onPositionChanged);
    AdvancedCpp::safeConnect(m_mediaPlayer.get(), &QMediaPlayer::durationChanged, this,
                             &VideoWidgetQt::onDurationChanged);
    AdvancedCpp::safeConnect(m_mediaPlayer.get(), &QMediaPlayer::errorOccurred, this,
                             &VideoWidgetQt::onErrorOccurred);

    // Connect video frame handler with threading using perfect forwarding
    AdvancedCpp::safeConnect(
        m_videoSink.get(), &QVideoSink::videoFrameChanged, this, [this](QVideoFrame frame) {
            if (frame.isValid()) {
                ...
                if (!labelSize.isEmpty() && !imageSize.isEmpty()) {
                    ...
                    AdvancedCpp::processFrame(
                        std::move(frame),
                        [this](auto&& f, auto&& size, auto&& quality) {
                            m_processingThread->processFrameGeneric(std::forward<decltype(f)>(f),
    ...
    AdvancedCpp::safeConnect(m_processingThread.get(), &VideoProcessingThread::frameProcessed, this,
                             [this](const QPixmap& pixmap) { m_videoLabel->setPixmap(pixmap); });
}
...
VideoWidgetQt::~VideoWidgetQt() {
    ...
    if (m_videoSink) { m_videoSink->disconnect(); }
    if (m_mediaPlayer) {
...
bool VideoWidgetQt::isPlaying() const {
    if (m_mediaPlayer) { return m_mediaPlayer->playbackState() == QMediaPlayer::PlayingState; }
    return false;
}

double VideoWidgetQt::getDuration() const { return m_duration; }
...
void VideoWidgetQt::onMediaStatusChanged(QMediaPlayer::MediaStatus status) {
    ...
    switch (status) {
        ...
        case QMediaPlayer::EndOfMedia:
            ...
            break;
        case QMediaPlayer::InvalidMedia: qDebug() << "Invalid media"; break;
        default: break;
    }
}
