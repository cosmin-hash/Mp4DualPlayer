...
MediaWidget::MediaWidget(QString displayName, QWidget* parent)
    : QWidget(parent), m_mainLayout(std::make_unique<QVBoxLayout>(this)),
      m_displayLabel(std::make_unique<QLabel>(this)), m_videoWidgetType(VideoWidgetType::Software),
      m_performanceLayout(std::make_unique<QHBoxLayout>()), m_qualityLabel(std::make_unique<QLabel>(this)),
      m_qualityCombo(std::make_unique<QComboBox>(this)), m_controlsLayout(std::make_unique<QHBoxLayout>()),
      m_openButton(std::make_unique<QPushButton>(this)), m_playButton(std::make_unique<QPushButton>(this)),
      m_pauseButton(std::make_unique<QPushButton>(this)), m_stopButton(std::make_unique<QPushButton>(this)),
      m_positionSlider(std::make_unique<QSlider>(Qt::Horizontal, this)),
      m_timeLabel(std::make_unique<QLabel>(this)), m_progressBar(std::make_unique<QProgressBar>(this)),
      m_positionTimer(std::make_unique<QTimer>(this)), m_fpsLabel(std::make_unique<QLabel>(this)),
      m_resolutionLabel(std::make_unique<QLabel>(this)), m_currentFile(""),
      m_displayName(std::move(displayName)), m_isPlaying(false), m_isPaused(false) {
...
void MediaWidget::setupUI() {
    ...
    if (currentVideoWidget) {
        ...
        qDebug() << "Video widget size policy:" << currentVideoWidget->sizePolicy().horizontalPolicy() << "x"
                 << currentVideoWidget->sizePolicy().verticalPolicy();
    ...
    for (QPushButton* btn :
         {m_openButton.get(), m_playButton.get(), m_pauseButton.get(), m_stopButton.get()}) {
...
void MediaWidget::setupConnections() {
    ...
    if (!m_connectionsSetup) {
        ...
        AdvancedCpp::safeConnect(m_playButton.get(), &QPushButton::clicked, this,
                                 &MediaWidget::onPlayClicked);
        AdvancedCpp::safeConnect(m_pauseButton.get(), &QPushButton::clicked, this,
                                 &MediaWidget::onPauseClicked);
        AdvancedCpp::safeConnect(m_stopButton.get(), &QPushButton::clicked, this,
                                 &MediaWidget::onStopClicked);

        // Slider and quality connections using perfect forwarding
        AdvancedCpp::safeConnect(m_positionSlider.get(), &QSlider::valueChanged, this,
                                 &MediaWidget::onSeekSliderChanged);
    ...
    if (m_videoWidgetQt) {
        AdvancedCpp::safeConnect(m_videoWidgetQt.get(), &VideoWidgetQt::videoLoaded, this,
                                 &MediaWidget::onVideoLoaded);
        AdvancedCpp::safeConnect(m_videoWidgetQt.get(), &VideoWidgetQt::playbackStarted, this,
                                 &MediaWidget::onPlaybackStarted);
        AdvancedCpp::safeConnect(m_videoWidgetQt.get(), &VideoWidgetQt::playbackPaused, this,
                                 &MediaWidget::onPlaybackPaused);
        AdvancedCpp::safeConnect(m_videoWidgetQt.get(), &VideoWidgetQt::positionChanged, this,
                                 &MediaWidget::onPositionChanged);
        AdvancedCpp::safeConnect(m_videoWidgetQt.get(), &VideoWidgetQt::durationChanged, this,
                                 &MediaWidget::onDurationChanged);
        AdvancedCpp::safeConnect(m_videoWidgetQt.get(), &VideoWidgetQt::fpsChanged, this,
                                 &MediaWidget::onFpsChanged);
        AdvancedCpp::safeConnect(m_videoWidgetQt.get(), &VideoWidgetQt::resolutionChanged, this,
                                 &MediaWidget::onResolutionChanged);
    }
...
void MediaWidget::openFile() {
    ...
    QString fileName =
        QFileDialog::getOpenFileName(this, tr("Open MP4 Video"), "", tr("MP4 Files (*.mp4);;All Files (*)"));
...
void MediaWidget::play() {
    if (!m_currentFile.isEmpty()) { playCurrentWidget(); }
}

void MediaWidget::pause() { pauseCurrentWidget(); }
...
void MediaWidget::seek(double position) {
    if (!m_currentFile.isEmpty()) { seekCurrentWidget(position); }
}

double MediaWidget::getCurrentPosition() const { return getCurrentPositionFromWidget(); }

double MediaWidget::getDuration() const { return getDurationFromWidget(); }

bool MediaWidget::isPlaying() const { return m_isPlaying; }

bool MediaWidget::isPaused() const { return m_isPaused; }

void MediaWidget::setMaxFrameRate(int fps) { setMaxFrameRateOnWidget(fps); }

void MediaWidget::setVideoQuality(int quality) { setVideoQualityOnWidget(quality); }
...
void MediaWidget::onPauseClicked() { pause(); }

void MediaWidget::onStopClicked() { stop(); }
...
void MediaWidget::updatePosition() {
    if (m_isPlaying && !m_isPaused) {
        ...
        if (duration > 0) {
            ...
            m_timeLabel->setText(
                QString("%1 / %2").arg(currentTime.toString("mm:ss")).arg(totalTime.toString("mm:ss")));
        }
    }
}
...
void MediaWidget::onPositionChanged(double position) {
    if (m_isPlaying && !m_isPaused) { updatePosition(); }
    emit positionChanged(position);
}

void MediaWidget::onDurationChanged(double duration) { emit durationChanged(duration); }

void MediaWidget::onFpsChanged(double fps) { m_fpsLabel->setText(QString("FPS: %1").arg(fps, 0, 'f', 1)); }
...
void MediaWidget::updateControls() {
    ...
    qDebug() << m_displayName << "controls updated - hasVideo:" << hasVideo << "isPlaying:" << m_isPlaying
             << "isPaused:" << m_isPaused;
}
...
QWidget* MediaWidget::getCurrentVideoWidget() const {
    if (m_videoWidgetQt) { return m_videoWidgetQt.get(); }
    return nullptr;
}

VideoWidgetQt* MediaWidget::getVideoWidgetQt() const { return m_videoWidgetQt.get(); }

void MediaWidget::setVideoWidgetType(VideoWidgetType type) {
    // Only software type is supported now
    if (m_videoWidgetType != type) {
        qDebug() << "Changing video widget type for:" << m_displayName << "from"
                 << static_cast<int>(m_videoWidgetType) << "to Software";
...
MediaWidget::VideoWidgetType MediaWidget::currentVideoWidgetType() const { return m_videoWidgetType; }
...
void MediaWidget::updateVideoWidgetInUI() {
    ...
    if (newVideoWidget) {
        ...
        qDebug() << "Video widget size policy:" << newVideoWidget->sizePolicy().horizontalPolicy() << "x"
                 << newVideoWidget->sizePolicy().verticalPolicy();
...
void MediaWidget::playCurrentWidget() {
    if (m_videoWidgetQt) { m_videoWidgetQt->play(); }
}

void MediaWidget::pauseCurrentWidget() {
    if (m_videoWidgetQt) { m_videoWidgetQt->pause(); }
}

void MediaWidget::stopCurrentWidget() {
    if (m_videoWidgetQt) { m_videoWidgetQt->stop(); }
}

void MediaWidget::seekCurrentWidget(double position) {
    if (m_videoWidgetQt) { m_videoWidgetQt->seek(position); }
}
...
void MediaWidget::setMaxFrameRateOnWidget(int fps) {
    if (m_videoWidgetQt) { m_videoWidgetQt->setMaxFrameRate(fps); }
}

void MediaWidget::setVideoQualityOnWidget(int quality) {
    if (m_videoWidgetQt) { m_videoWidgetQt->setVideoQuality(quality); }
}
